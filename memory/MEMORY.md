...
- [🚨무배는 마크업 배수 먼저](feedback_free_shipping_markup_check.md) — 배송원가를 "못 받은 돈"으로 세지 말 것, 계산 전에 적자 우려 금지(3회 반복). order_settlement.py 가 경고 출력
...
- [Todoist 배치이동=reparent만](feedback_todoist_move_not_delete_recreate.md) — 삭제후재생성 금지, 완료태스크는 uncomplete→move→complete
- [한미 진행 멈추지 말것](feedback_hanmi_proceed_without_stalling.md) — 트래킹번호 보여준 것 자체가 확인, 재확인 대기 없이 매칭·파이널체크·Slack까지 진행
- [네이버 AI생성물정책](feedback_naver_ai_content_policy_2026.md) — 2026-07-10시행, 이미지/영상 대상 확인(3소스 원문교차), 텍스트카피는 대상아닌쪽으로 잠정확인, 공지원문 미확보라 완전종결은 아님
...
- [Notion 정산차트](feedback_settlement_notion_chart.md) — 제품별 주문수(count)+월정산, 개수=실주문량; 업데이트 후 항상 Codex QA로 (a)제품명 중복 (b)프로퍼티 포맷 (c)자유서술 오탈자·비문 검증
...
- [정산 특이사항 노션기록](feedback_settlement_notable_notes.md) — 배치 특이사항 자동기록, 한글 유니코드 이스케이프 금지, "버그 발견" 류는 실제 정산파일 검산 후 서술
...
- [AI검색 대량등록페널티+랭킹개편](feedback_ai_search_2026_bulk_penalty.md) — 카테고리 한단계 차이도 다른유형, 등록한도(확인됨), 8/13 광고축소/실반응 랭킹개편(전자신문)
