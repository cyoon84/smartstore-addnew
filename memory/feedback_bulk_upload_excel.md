...
**🔑 단위가격(단가) 표시제 — 갭, 스크립트 미구현 (2026-07-25 정책 스터디 확인, LEARNED §16-4):** 2026-04-29 시행, 이미 발효 중. 식품·생활건강·화장품·유아용품 등 525개 카테고리는 **기준단위가격**(100g/100ml당 등) 입력이 조건부필수 — 안 넣으면 그 상품 **수정 자체가 차단**된다. `ExcelSaveTemplate_*.xlsx` 2행에 `단위가격 사용여부`·`표시용량`·`표시단위`·`총용량` 컬럼이 이미 있는데(조건부필수 표시), `build_bulk_excel.py` 는 `unit_price_use="N"` 고정이고 나머지 3개는 매핑 안 함 — 식품 SKU 비중 큰 우리 카탈로그엔 실질 갭. 예외(30g/30ml 이하·500원 이하)도 있어 자동 로직 구현 전에 **어느 카테고리가 대상인지·표시용량=실용량 그대로인지 100단위 환산인지**를 사장님 확인 후 진행(Slack 제안 상태, 아직 스크립트 미변경).

**🔑 계산식·예외 확정 (2026-08-07 후속, 3소스 교차확인):** `단위가격 = 판매가 ÷ 총용량 × 표시용량`. **총용량 = 개당 용량이 아니라 그 판매단위(묶음) 전체 총 용량** — 예 `1.5L 생수 6개입`을 100ml 기준으로 표시하면 총용량=**9,000**(개당 1500 아님). 멀티팩/묶음 SKU 는 이 함정 주의. 예외 4종 확정: ①30g/30ml 이하 초소용량 ②총판매가 500원 이하 ③여러 상품 복합포장 ④1개 규격뿐인 단일상품. 미입력 시 "상품 전송(등록/수정) 자체가 실패"(플레이오토 확인, §16-4 "수정제한"과 일치). 525개 카테고리 전체 리스트는 여전히 미확보 — **어느 SKU가 대상인지 자동판별하는 로직은 계속 보류**, 계산식만 확정돼 사람이 직접 채울 때는 이제 바로 적용 가능.

**🔑 "미성년자 구매" 필드(33번 컬럼) — 공란이면 자동 Y(구매가능), 우리 카탈로그는 손댈 필요 없음 (2026-09-02 검증):** 외부 블로그(tokki-ai)가 "미성년자 구매 여부 누락 시 식품·화장품에서 자동 차단된다"고 주장해 실제 템플릿(`ExcelSaveTemplate_*.xlsx` 2행 가이드)을 대조 — **정반대였다**: "미성년자 구매여부가 공란인 경우 **Y(구매가능)**로 등록됩니다"가 원문. `build_bulk_excel.py` 는 이 칸을 안 채우고 있지만(비필수), 우리는 주류·담배·성인용품을 전혀 취급하지 않으므로 공란=Y 기본값이 안전. **스크립트 변경 불필요, 외부 주장 쪽이 부정확했음을 확인.** 같은 글의 모델명(검토필요 70% 원인, §5 이미 반영)·KC인증(§16 이미 반영)·원산지(§16 origin_code 고정으로 이미 정확값)·AS정보(§16 CONFIG 기본값 이미 존재)는 전부 기존 규칙과 일치, 갭 없음.
