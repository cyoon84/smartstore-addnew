...
**🔑 카피·이미지 최적화 3종 (2026-07-16 학습, §17-1 보강):** ①핵심포인트 설명을 **기능→결과·시간·숫자로 번역**(❌"강력한 모터" ✅"6초면 끝", 단 정량수치는 검증출처 있을 때만 §9). ②**정량 사회적 증거(재구매율·리뷰별점·후기수)를 상세 여러 지점에 반복** 배치 — 리셀러라 초기엔 데이터 없으니 포토리뷰 확보되는 대로(날조 금지, 실제값 0이면 비움). ③**호스팅 이미지 용량/포맷/세로분할 최적화**(가로860·세로~1,100px 분할·JPEG=라이프스타일/PNG=제품컷·압축, 모바일 로딩=체류). alt는 "토픽 서술 125자, 키워드 나열 금지"로 업그레이드. 출처 weolbu·converta·ascentkorea. [[project_daily_smartstore_study]]

**🔑 이모지 절제 (2026-07-21, "AI티 난다"):** 말풍선 섹션 헤더(핵심 포인트·제품 정보·브랜드 소개 등)엔 이모지 금지 — 텍스트만. 한 헤더만 국기/심볼 붙이면 나머지와 어긋나 AI티. 이모지는 핵심포인트 항목당 1개·훅 끝 1개로만, 한 자리 2개 겹치기(🍑🍵) 금지. 아이브로우 원산지 국기(🇨🇦)는 관용 유지.

**🔑 카피 자연스러움 (2026-07-21):** 상세 카피에 억지 소구·군더더기 상투어 금지. (a)사실 어긋난 억지 문구 금지(예 "카페에서만 즐기던 피치 말차"—피치말차는 카페메뉴 아님→"말차라떼"로). (b)`국내에 흔치 않은 캐나다 직수입`·희소성 상투어를 상품명/아이브로우에 이미 썼는데 핵심포인트·좋은점에 중복 금지. (c)서술 문장 끝에 "· 직수입 65g" 스펙 꼬리표 붙이기 금지(스펙은 제품정보 블록에만). 항목 줄면 억지로 안 채움. [[feedback_senior_copywriting_mindset]]

**🔑 근거 없는 희소성 문구 금지 (2026-07-21):** `국내에 흔치 않은`·`국내 미출시`·`희귀` 류를 검증 없이 쓰지 말 것(사용자 "앞으로 이런거 근거없이 쓰지마"). §0-A로 실제 국내 미유통 확인된 경우만, 그때도 1회. 애매하면 실제 제품 특징(낱개 포장·용량·성분)으로 대체. 리세스 빅컵 상세에서 "국내엔 흔치 않은 사이즈" 3곳 정정.

**🔑 GIF는 별도 마크업 없이 기존 `<img>` 로 이미 가능 (2026-08-02, 1분상세·키위스냅 정독):** 사용법·개폐방식 같은 동적 정보는 GIF 로 표현하면 효과적이라는 팁 — 우리 §7/§17 허용 태그가 `<img src="URL">` 이고 GIF 도 이미지 포맷의 하나라 **호스팅 URL 이 `.gif` 면 그대로 넣으면 된다**(새 규칙 아님, 기존 허용범위 활용 노트). `<video>`/embed 계열은 여전히 금지 대상 태그라 미검증 — 시도 전 사용자 확인. 가로860·세로~1,100px분할·20MB상한(§17-1)과 6단계 콘텐츠 흐름(특장점→입증→시각화→디테일→상황카피→리뷰)은 기존 §17/§17-1 구조와 이미 동형이라 갭 없음. [[project_daily_smartstore_study]]

**🔑 후기 인용 = "만족했어요" 정성 대신 구체 프로필 붙이기 (2026-08-12, 톡키AI `dwell-time-detail-page-restructuring-2026` 정독):** §17-1 요소5(정량 사회적 증거)의 실행 디테일 — 실제 포토리뷰를 확보해 상세에 인용할 때, 단순 만족도 문구 대신 **"168cm 65kg, 캠핑 6년 차인데…"처럼 구매자 프로필(키·체중·사용경험·용도)을 붙여서 인용**하면 신뢰도·체류시간이 더 오른다는 팁. §9 데이터 진실성 원칙 그대로 유지 — **실제 리뷰 원문에 프로필 정보가 있을 때만** 그대로 옮기고, 없는 프로필을 지어내지 않는다(리뷰 자체가 아직 없으면 이 요소는 비워둠, 기존 방침과 동일). 같은 글의 "배송/AS 예외비용(도서산간 추가배송비 등) 상세 본문에 명시" 제안은 **채택 안 함** — [[feedback_detail_skip_shipping_outro]] 이 배송/교환/반품 정보를 상세 본문에서 빼도록 정한 이유(네이버 상품페이지가 그 영역을 이미 자동 노출 → 중복 방지)가 여전히 유효해 상충 규칙으로 보류. 모바일 폰트(14px+)·이미지세로(600~800px) 권장치는 우리 기존 스펙(본문 24~26px·세로~1,100px)이 이미 상회해 갭 없음.
