...
**🔑 우버 그로서리 reimburse 건은 이메일 제목으로 신호 (2026-07-29 사용자 확정):** `chulhee.y+receipt@gmail.com` 로 영수증을 보낼 때 사용자가 **제목(subject)에 "우버 그로서리"/"uber grocery"**라고 직접 적기로 함 — 이 신호가 있으면 **품목 내용을 보고 COGS/식비/차량정비 등으로 추측하지 말고 무조건 `우버이츠 reimburse 대기` 탭**으로 기입한다(대소문자 무관). 이전엔 품목명(모터오일 등)만 보고 다른 탭에 넣었다가 사용자가 정정 — 제목 신호가 있으면 그게 최우선 판단 근거. receipt-plus-address-poller 스케줄 작업 SKILL.md 4단계에 반영 완료.
...
**🔑 `수입 (RBC)` 탭 채우기 = 사용자가 정기적으로 RBC 명세서(PDF) 던져주는 방식 (2026-08-04 확정):** 입금(매출)은 영수증처럼 자동수집 채널이 없어서, 자동화(CSV 연동 등) 대신 **사용자가 주기적으로 RBC business account statement PDF 를 그대로 채팅에 올리면 그때 파싱해서 채우는 수동-트리거 방식**으로 가기로 확정. 명세서 받으면: ① `pdfminer.high_level.extract_text` 로 파싱 ② `Account Activity Details` 라인별로 Naver(`TT1/NAVERFINA`)·우버(`Uber Holdings`) 입금은 `수입 (RBC)` 탭에 기입(송금수수료는 별개 행 음수로), 월 계좌수수료(`Monthly fee`)는 `Supply` 탭 ③ AMEX/CIBC 등 **카드대금 결제(`AMEX BILL PYMT`·`Online Banking payment...MASTERCARD`)는 이미 개별 구매로 잡혀있던 걸 갚는 것이라 새 지출로 넣지 않음**(이중집계 방지) ④ 개인 e-Transfer 왕복은 사용자에게 성격 확인(가지급금/상환 vs 사업비 reimbursement) — Interac 알림메일(`notify@payments.interac.ca`)의 "Message:" 필드에 실제 사유가 적혀있으니 애매하면 Gmail 검색으로 대조. 이번 첫 적용(6/26~7/27 명세서)에서 $1,900 왕복=주주가지급금(상환완료), $330.19($285.32+$44.87)=개인카드로 먼저 낸 사업비 reimbursement로 확인됨 — 둘 다 P&L 미포함.
